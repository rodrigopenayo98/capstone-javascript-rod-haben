import { saveComment, fetchShowComments } from './apiComments.js';

const modal = document.getElementById('modal');
const modalBody = document.getElementById('modal-body');

const countNumberOfShows = (countDomm, shows = []) => {
  countDomm.innerText = shows.length;
};
...
const modalTemplate = (show) => `     
                ...
                <div class="comments">
                  ...
                  <ul class="comments-list">
                        ${show.comments.map((comment) => `<li class="comment-item">${comment.username} : ${comment.comment} </li>`).join(' ')}
                  </ul>
                </div>
        `;
